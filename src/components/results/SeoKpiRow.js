...
export default function SeoKpiRow({ result }) {
  ...
  const sitemap = typeof result?.sitemap?.present === 'boolean'
    ? result.sitemap.present
    : (typeof result?.technical?.sitemap === 'boolean' ? result.technical.sitemap
    : (typeof result?.summary?.sitemapFound === 'boolean' ? result.summary.sitemapFound : null));
  const robotsTxt = typeof result?.robotsTxt?.present === 'boolean'
    ? result.robotsTxt.present
    : (typeof result?.technical?.robotsTxt === 'boolean' ? result.technical.robotsTxt
    : (typeof result?.summary?.robotsFound === 'boolean' ? result.summary.robotsFound : null));
